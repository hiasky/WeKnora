...
| POST | `/auth/register-by-invite` | 通过邀请链接注册                           |
| POST | `/auth/invitations/lookup` | 查询邀请上下文（token → 租户/角色）        |
| POST | `/auth/login`              | 用户登录                                   |
| POST | `/auth/auto-setup`         | Lite 桌面版自动初始化                      |
| GET  | `/auth/config`             | 获取认证配置（注册模式）                   |
| POST | `/auth/switch-tenant`      | 切换激活租户                               |
...
| PUT  | `/auth/me/preferences`     | 更新当前用户偏好设置                       |
...
## POST `/auth/register-by-invite` - 通过邀请注册

使用分享链接中的 token 注册新用户。无需认证。token 携带邀请上下文（租户、角色）。
...
```curl
curl --location 'http://localhost:8080/api/v1/auth/register-by-invite' \
--header 'Content-Type: application/json' \
--data '{
...
**响应**（201 Created）：同 `/auth/register`，返回 user、tenant、token 信息。自动加入邀请指定的租户和角色。

**错误**: token 无效或已过期 → 410 Gone；参数校验失败 → 400。

---

## POST `/auth/invitations/lookup` - 查询邀请上下文

根据邀请链接中的 token 返回邀请上下文（租户名、角色、过期时间），供注册页面展示。无需认证。

> 使用 POST + body 而非 GET + path，避免 token 落入访问日志、浏览器历史和分布式追踪 span 中。
...
```curl
curl --location 'http://localhost:8080/api/v1/auth/invitations/lookup' \
--header 'Content-Type: application/json' \
--data '{
    "token": "invite-token-xxxxx"
}'
...
```json
{
    ...
    "data": {
        ...
        "tenant_name": "研发团队工作区",
        "role": "editor",
        "expires_at": "2025-08-19T10:00:00+08:00"
    }
}
```

**错误**: token 无效或已撤销 → 410 Gone。

---

## POST `/auth/auto-setup` - 自动初始化（Lite 桌面版）

**Lite 版专用**：首次启动时自动创建默认用户和租户并返回令牌，后续启动直接签发令牌，无需手动注册/登录。

**请求**:

```curl
curl --location --request POST 'http://localhost:8080/api/v1/auth/auto-setup'
```

**响应**（200 OK）: 同 `/auth/login`，返回 user、tenant、token、refresh_token。
...
返回当前部署的注册模式等公开认证配置，供前端决定是否展示注册入口。**无需认证**。

**请求**:

```curl
curl --location 'http://localhost:8080/api/v1/auth/config'
```
...
`registration_mode` 取值：

| 值 | 说明 |
| ---- | ---- |
| `open` | 开放注册（默认） |
| `invite_only` | 仅邀请注册 |
| `disabled` | 注册已关闭（通过 `DISABLE_REGISTRATION=true` 环境变量） |

---

## POST `/auth/switch-tenant` - 切换激活租户

为当前用户签发目标租户的访问令牌。用户必须在目标租户有 active 成员关系。需要 Bearer JWT 认证。

**参数说明（请求体）**:

| 字段 | 类型 | 必填 | 说明 |
| ---- | ---- | ---- | ---- |
| `tenant_id` | uint64 | 是 | 目标租户 ID |
| `refresh_token` | string | 否 | 当前的 refresh_token（用于生成新的 refresh_token） |

**请求**:

```curl
curl --location 'http://localhost:8080/api/v1/auth/switch-tenant' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer eyJhbGciOi...' \
--data '{
    "tenant_id": 2,
...
**响应**（200 OK）: 同 `/auth/login` 返回的 LoginResponse（含新的 access_token、refresh_token、user、tenant）。

**错误**: 非该租户成员 → 403。

---

## PUT `/auth/me/preferences` - 更新用户偏好

按 PATCH 语义合并用户偏好（仅覆盖请求体里出现的字段）。数据存储在 `users.preferences` (JSON)，跨设备/浏览器自动同步。需要 Bearer JWT 认证。

**参数说明（请求体）**:

| 字段 | 类型 | 必填 | 说明 |
| ---- | ---- | ---- | ---- |
| `enable_memory` | bool | 否 | 是否启用对话记忆功能 |
| `last_active_tenant_id` | uint64 | 否 | 上次活动租户 ID（传 0 清除，传 nil 保持不变） |
